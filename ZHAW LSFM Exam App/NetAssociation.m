...
- (NSData *) createPacket {
	...
	ntpClientSendTime.fullSeconds = (int)now.tv_sec + JAN_1970; ///busi
...
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  ...
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘*/
    ...
    ntpClientRecvTime.fullSeconds = (int)arrival_time.tv_sec + JAN_1970;  ///busi   // Transmit Timestamp coarse
...
- (void) evaluatePacket {
    double          packetOffset = INFINITY;                     // initial untrustworthy offset
...
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  ...
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘*/
    if ((dispersion > 0.001 && dispersion < 50.0) && (stratum > 0) && (mode == 4) &&
    ...
    ///NTP_Logging(@"  [%@] {%3.1f,%3.1f,%3.1f,%3.1f,%3.1f,%3.1f,%3.1f,%3.1f}", server, fifoQueue[0], fifoQueue[1], fifoQueue[2], fifoQueue[3], fifoQueue[4], fifoQueue[5], fifoQueue[6], fifoQueue[7]);
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
...
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  ...
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘*/
    if (good > 0 || fail > 3) {
        ...
        ///NTP_Logging(@"%@ [%@] {good: %i; fail: %i; none: %i} offset=%3.1fmS ", (trusty) ? @"↑" : @"↓", server, good, fail, none, offset);
...
/*┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
  ...
  └──────────────────────────────────────────────────────────────────────────────────────────────────┘*/
    ...
    if ([repeatingTimer timeInterval] != pollIntervals[pollingIntervalIndex]) {
        ///NTP_Logging(@"[%@] poll interval adusted: %3.1f >> %3.1f", server,[repeatingTimer timeInterval], pollIntervals[pollingIntervalIndex]);
...
- (NSString *) prettyPrintPacket {
    ...
    [prettyString appendFormat:@"  clock last set: %u.%.6u (%@)\n",
                        (unsigned int)ntpServerBaseTime.fullSeconds, Frac2uSec(ntpServerBaseTime.partSeconds),
                        [self dateFromNetworkTime:&ntpServerBaseTime]];
    [prettyString appendFormat:@"client send time: %u.%.6u (%@)\n",
                        (unsigned int)ntpClientSendTime.fullSeconds, Frac2uSec(ntpClientSendTime.partSeconds),
